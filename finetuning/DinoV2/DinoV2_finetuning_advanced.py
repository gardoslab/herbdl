...
""" Fine-tuning a 🤗 Transformers DinoV2 model for image classification with advanced augmentations"""
...
class MultiTaskDinoV2Model(nn.Module):
    """
    Multi-task DinoV2 model with separate classification heads for family, genus, and species.
    """
    def __init__(self, base_model, num_families, num_genera, num_species):
        ...
        # Extract the base DinoV2 encoder
        if hasattr(base_model, 'dinov2'):
            self.dinov2 = base_model.dinov2
        else:
            raise ValueError("Base model must have a 'dinov2' attribute")
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(description="DinoV2 Fine-tuning with advanced augmentations")
    ...
    if use_multi_task:
        print("__CUSTOM__: Creating multi-task DinoV2 model")
    ...
    if frozen:
        ...
        # DinoV2-base param naming: dinov2.embeddings.*, dinov2.encoder.layer.{0..11}.*,
        # dinov2.layernorm.*, classifier.*. Tiered unfreezing of trailing layers (12 total).
        for name, param in model.named_parameters():
            if frozen_type == "v1":
                if 'classifier' not in name and "dinov2.layernorm" not in name:
                    param.requires_grad = False
            elif frozen_type == "v4":
                if 'classifier' not in name and "dinov2.layernorm" not in name and not name.startswith("dinov2.encoder.layer.11") and not name.startswith("dinov2.encoder.layer.10") and not name.startswith("dinov2.encoder.layer.9"):
                    param.requires_grad = False
            elif frozen_type == "v3":
                if 'classifier' not in name and "dinov2.layernorm" not in name and not name.startswith("dinov2.encoder.layer.11") and not name.startswith("dinov2.encoder.layer.10"):
                    param.requires_grad = False
            else:
                if 'classifier' not in name and "dinov2.layernorm" not in name and not name.startswith("dinov2.encoder.layer.11"):
                    param.requires_grad = False
