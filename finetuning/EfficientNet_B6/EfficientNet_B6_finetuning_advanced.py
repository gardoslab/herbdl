...
""" Fine-tuning a 🤗 Transformers EfficientNet B6 model for image classification with advanced augmentations"""
...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default="google/efficientnet-b6",
...
class MultiTaskEfficientNetModel(nn.Module):
    """
    Multi-task EfficientNet model with separate classification heads for family, genus, and species.
    """
    def __init__(self, base_model, num_families, num_genera, num_species):
        ...
        # Extract the base EfficientNet encoder
        if hasattr(base_model, 'efficientnet'):
            self.efficientnet = base_model.efficientnet
        else:
            raise ValueError("Base model must have an 'efficientnet' attribute")

        # Get hidden size from config (EfficientNet uses hidden_dim)
        hidden_size = getattr(base_model.config, 'hidden_dim', None) or base_model.config.hidden_size
    ...
    def forward(self, pixel_values, family_labels=None, genus_labels=None, species_labels=None, **kwargs):
        ...
        # EfficientNet's pooler_output may have spatial dims of size 1 - flatten if needed
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(description="EfficientNet B6 Fine-tuning with advanced augmentations")
    ...
    if 'learning_rate_kwargs' in config['training'] and config['training']['learning_rate_kwargs']:
        training_args.learning_rate_kwargs = config['training']['learning_rate_kwargs']
    ...
    if use_multi_task:
        print("__CUSTOM__: Creating multi-task EfficientNet model")
        ...
        model = MultiTaskEfficientNetModel(
            base_model,
    ...
    if frozen:
        # EfficientNet parameter naming pattern:
        #   efficientnet.embeddings.*
        #   efficientnet.encoder.blocks.{i}.*  (i = 0..N-1)
        #   efficientnet.encoder.top_conv.*, efficientnet.encoder.top_bn.*
        #   classifier.*
        # Freezing tiers (relative to total block count):
        #   v1 -> only classifier trainable
        #   v2 -> classifier + top_conv/top_bn
        #   v3 -> classifier + top + last 25% of blocks
        #   v4 -> classifier + top + last 50% of blocks
        print("__CUSTOM__: Freezing model according to the frozen type: ", frozen_type)

        encoder_holder = model.efficientnet if hasattr(model, 'efficientnet') else model
        total_blocks = len(encoder_holder.encoder.blocks)

        if frozen_type == "v1":
            unfreeze_block_threshold = total_blocks
            unfreeze_top = False
        elif frozen_type == "v2":
            unfreeze_block_threshold = total_blocks
            unfreeze_top = True
        elif frozen_type == "v3":
            unfreeze_block_threshold = int(total_blocks * 0.75)
            unfreeze_top = True
        else:  # v4 (and any unknown frozen_type)
            unfreeze_block_threshold = int(total_blocks * 0.5)
            unfreeze_top = True

        block_idx_re = re.compile(r'encoder\.blocks\.(\d+)\.')
        for name, param in model.named_parameters():
            if 'classifier' in name:
                continue
            if unfreeze_top and ('top_conv' in name or 'top_bn' in name):
                continue
            m = block_idx_re.search(name)
            if m and int(m.group(1)) >= unfreeze_block_threshold:
                continue
    ...
    if use_advanced_aug:
        ...
        if aug_config.get('random_erasing', {}).get('enabled', False):
            re_cfg = aug_config['random_erasing']
            train_transform_list.append(
                RandomErasing(
                    p=re_cfg.get('probability', 0.25),
                    scale=(re_cfg.get('min_area', 0.02), re_cfg.get('max_area', 0.33)),
                )
            )
