...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default="timm/coatnet_3_rw_224.sw_in12k",
...
class MultiTaskCoAtNetModel(nn.Module):
    """
    Multi-task CoAtNet model with separate classification heads for family, genus, and species.
    """
    def __init__(self, base_model, num_families, num_genera, num_species):
        ...
        # CoAtNet via TimmWrapper exposes the raw timm module as base_model.timm_model
        if hasattr(base_model, 'timm_model'):
            self.timm_model = base_model.timm_model
        else:
            raise ValueError("Base model must have 'timm_model' attribute (TimmWrapper)")

        # timm exposes num_features on every model; fall back to config.hidden_size if needed
        hidden_size = getattr(self.timm_model, 'num_features', None) or base_model.config.hidden_size
    ...
    def forward(self, pixel_values, family_labels=None, genus_labels=None, species_labels=None, **kwargs):
        # timm's forward_head with pre_logits=True returns the pooled feature vector before the classifier
        features = self.timm_model.forward_features(pixel_values)
        pooled_output = self.timm_model.forward_head(features, pre_logits=True)
...
class MixupCutmixCollator:
    ...
    def __call__(self, examples):
        ...
        # Only flip the coin between augmentations that are actually enabled (alpha > 0).
        # Flipping unconditionally would let an isolated run (e.g. cutmix-only) land on the
        # disabled augmentation and silently no-op ~half the time, applying it at half the
        # intended rate and skewing isolated-vs-isolated sweep comparisons.
        mixup_on = self.mixup_alpha > 0
        cutmix_on = self.cutmix_alpha > 0
        if mixup_on and cutmix_on:
            ...
            # Neither augmentation enabled: return the clean batch unchanged.
            result = {"pixel_values": pixel_values, "labels": labels}
            if self.multi_task:
                result.update({
                    "family_labels": family_labels,
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(description="CoAtNet Fine-tuning with advanced augmentations")
    ...
    if use_multi_task:
        print("__CUSTOM__: Creating multi-task CoAtNet model")
    ...
    # TimmWrapperImageProcessor exposes data_config (timm-style) instead of size/image_mean/image_std (HF-style). Shim missing attrs so the existing transform pipeline below is unchanged.
    if hasattr(image_processor, "data_config") and not hasattr(image_processor, "size"):
        _dc = image_processor.data_config
        _c, _h, _w = _dc["input_size"]
        image_processor.size = {"height": _h, "width": _w}
        image_processor.image_mean = list(_dc["mean"])
        image_processor.image_std = list(_dc["std"])

    if frozen:
        print("__CUSTOM__: Freezing model according to the frozen type: ", frozen_type)
        # CoAtNet (timm) param naming: timm_model.stem.*, timm_model.stages.{0..3}.*, timm_model.head.*. Stages 0–1 are conv blocks, 2–3 are transformer blocks.
        for name, param in model.named_parameters():
            if frozen_type == "v1":
                if 'classifier' not in name and "timm_model.head" not in name:
                    param.requires_grad = False
            elif frozen_type == "v4":
                if 'classifier' not in name and "timm_model.head" not in name and not name.startswith("timm_model.stages.3") and not name.startswith("timm_model.stages.2") and not name.startswith("timm_model.stages.1"):
                    param.requires_grad = False
            elif frozen_type == "v3":
                if 'classifier' not in name and "timm_model.head" not in name and not name.startswith("timm_model.stages.3") and not name.startswith("timm_model.stages.2"):
                    param.requires_grad = False
            else:
                if 'classifier' not in name and "timm_model.head" not in name and not name.startswith("timm_model.stages.3"):
                    param.requires_grad = False
