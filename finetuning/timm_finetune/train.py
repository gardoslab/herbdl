...
""" Generic image-classification fine-tuner (multi-task family/genus/species, balanced softmax,
EMA, mixup/cutmix, multi-crop+flip TTA) supporting BOTH HuggingFace SWIN backbones and
**timm** domain-pretrained backbones (e.g. ConvNeXt-V2 / EVA-02 pretrained on iNaturalist).

Forked from finetuning/SWIN/SWIN_finetuning_advanced.py with a timm backbone path added; the
backbone-agnostic machinery (collator, trainer, EMA, eval loop, metrics) is reused unchanged.
Select the backbone with `model.backbone_type: hf|timm` in the YAML config. """
...
class ModelArguments:
    ...
    backbone_type: str = field(
        default="hf",
        metadata={"help": "Backbone loader: 'hf' (AutoModelForImageClassification) or 'timm' (timm.create_model)."},
    )
    img_size: Optional[int] = field(
        default=None,
        metadata={"help": "Input size for the timm backbone (e.g. 384). Ignored for the 'hf' path."},
    )


def _build_timm_config(model_args, feat_dim, preproc):
    """Lightweight HF PretrainedConfig so the wrapper can save a recoverable config.json
    (carries everything needed to rebuild the timm backbone + preprocessing for inference)."""
    ...
    return PretrainedConfig(
        ...
        backbone_type="timm",
        backbone_name=model_args.model_name_or_path,
        image_size=[h, w],
        image_mean=list(preproc["mean"]),
        image_std=list(preproc["std"]),
        model_type="timm_backbone",
    )


def build_backbone(model_args, num_labels_for_hf_config):
    """
    Build a backbone behind a uniform interface. Returns (backbone, feat_dim, preproc, hf_config):
      - backbone : nn.Module. HF -> the AutoModelForImageClassification (the wrappers extract its
        .swin/.swinv2 encoder + read .pooler_output, unchanged). timm -> timm.create_model(
        ..., num_classes=0), whose forward returns a pooled [B, feat_dim] tensor directly.
      - feat_dim : pooled feature dim.
      - preproc  : {"size": int|(h,w), "mean": [...], "std": [...]} for timm; None for HF
        (HF preprocessing comes from AutoImageProcessor downstream).
      - hf_config: AutoConfig (HF) or None (timm).
    """
    if model_args.backbone_type == "timm":
        import timm
        from timm.data import resolve_model_data_config
        name = model_args.model_name_or_path
        # iNat21 checkpoints live on the HF hub (e.g. timm/eva02..._ft_inat21,
        # BBracke/convnextv2_base.inat21_384) and are NOT in timm's built-in registry, so a
        # repo id with a "/" must be loaded via the hf-hub mechanism. Bare names (e.g.
        # "convnextv2_base.fcmae_ft_in22k_in1k_384") load from the registry directly.
        if name.startswith("hf-hub:"):
            pass
        elif "/" in name:
            name = "hf-hub:" + name
        create_kwargs = dict(pretrained=True, num_classes=0)
        if model_args.img_size:
            create_kwargs["img_size"] = model_args.img_size
        try:
            backbone = timm.create_model(name, **create_kwargs)
        except TypeError:
            # Some archs (e.g. ConvNeXt) don't accept img_size; the data config still reports
            # the checkpoint's native input size.
            create_kwargs.pop("img_size", None)
            backbone = timm.create_model(name, **create_kwargs)
        feat_dim = backbone.num_features
        dc = resolve_model_data_config(backbone)
        _, h, w = dc["input_size"]
        preproc = {"size": (h, w), "mean": list(dc["mean"]), "std": list(dc["std"])}
        print(f"__CUSTOM__: timm backbone '{name}' loaded — num_features={feat_dim}, "
              f"input={h}x{w}, mean={preproc['mean']}, std={preproc['std']}")
        return backbone, feat_dim, preproc, None

    # --- HF path (unchanged behavior) ---
    config_obj = AutoConfig.from_pretrained(
        model_args.config_name or model_args.model_name_or_path,
        num_labels=num_labels_for_hf_config,
    ...
    return base_model, config_obj.hidden_size, None, config_obj


class MultiTaskModel(nn.Module):
    """
    Multi-task model with family/genus/species heads on a shared backbone.
    Backbone is either an HF SWIN/SWINv2 (extract encoder, read pooler_output) or a timm
    model (num_classes=0, returns the pooled vector directly). Head logic is identical.
    """
    def __init__(self, base_model, num_families, num_genera, num_species,
                 family_weight=0.2, genus_weight=0.3, species_weight=1.0,
                 feature_dim=None, is_timm=False, wrapper_config=None):
        super().__init__()
        ...
        if is_timm:
            # timm backbone: forward returns a pooled [B, feature_dim] vector directly.
            self.backbone = base_model
            hidden_size = feature_dim
            self.config = wrapper_config            # lightweight PretrainedConfig (Trainer needs .config)
        else:
            # HF SWIN/SWINv2: extract encoder, read pooler_output.
    ...
    def _pooled(self, pixel_values):
        """Return the [B, hidden_size] pooled feature vector for either backbone type."""
        if self.is_timm:
            return self.backbone(pixel_values)
        return self.swin(pixel_values).pooler_output
    ...
    def forward(self, pixel_values, family_labels=None, genus_labels=None, species_labels=None, **kwargs):
        pooled_output = self._pooled(pixel_values)  # [batch_size, hidden_size]
...
class MixupTrainer(Trainer):
    ...
    def __init__(self, *args, multi_task=False, arcface=False,
        ...
        # kept in sync with MultiTaskModel's internal weighting).
        self.family_weight = family_weight
        self.genus_weight = genus_weight
        self.species_weight = species_weight
...
def _relocate_output_dir(path):
    ...
    Configs hardcode paths like
    /projectnb/herbdl/workspaces/<author>/herbdl/finetuning/output/<...>/<NAME>.
    ...
    The trailing run name is preserved. No-op if the path doesn't match that layout
    or is already under this repo. Set HERBDL_NO_RELOCATE=1 to disable.
    """
    if not isinstance(path, str) or not path or os.environ.get('HERBDL_NO_RELOCATE'):
        return path
    # repo root = three levels up from this file: .../herbdl/finetuning/<module>/<file>
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        description="Image-classification fine-tuning (timm or HF backbones) with multi-task "
                    "heads, balanced softmax, EMA, mixup/cutmix, and multi-crop TTA")
    ...
    model_args = ModelArguments(
        ...
        backbone_type=config['model'].get('backbone_type', 'hf'),
        img_size=config['model'].get('img_size', None),
    )
    ...
    preproc = None  # set by the timm path; None means HF preprocessing via AutoImageProcessor
    if use_arcface:
        if model_args.backbone_type == "timm":
            raise NotImplementedError(
                "timm backbones currently support multi-task CE only; ArcFace is not wired for "
                "timm. Set arcface.enabled: false and multi_task.enabled: true."
            )
        ...
        print(f"__CUSTOM__: Creating multi-task model (backbone_type={model_args.backbone_type})")

        # Build backbone (HF SWIN or timm) behind a uniform interface
        backbone, feat_dim, preproc, hf_cfg = build_backbone(model_args, num_labels_for_hf_config=num_species)
        wrapper_cfg = hf_cfg if hf_cfg is not None else _build_timm_config(model_args, feat_dim, preproc)
        ...
        if model_args.backbone_type == "timm":
            raise NotImplementedError(
                "timm backbones currently support multi-task CE only; set multi_task.enabled: true."
            )
        config_obj = AutoConfig.from_pretrained(
            model_args.config_name or model_args.model_name_or_path,
            num_labels=len(labels),
    ...
    # timm provides its own preprocessing (size/mean/std via `preproc`); HF uses AutoImageProcessor.
    ...
    # Preprocessing source: timm `preproc` dict (timm path) or the HF AutoImageProcessor.
    if preproc is not None:
        size = preproc["size"]
        proc_mean, proc_std = preproc["mean"], preproc["std"]
        normalize = Normalize(mean=proc_mean, std=proc_std)
    else:
        ...
        if hasattr(image_processor, "image_mean") and hasattr(image_processor, "image_std"):
            proc_mean, proc_std = image_processor.image_mean, image_processor.image_std
            normalize = Normalize(mean=proc_mean, std=proc_std)
        else:
            proc_mean, proc_std = [0.0, 0.0, 0.0], [1.0, 1.0, 1.0]
